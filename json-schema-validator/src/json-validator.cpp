...
class schema {
 ...
 public:
  ...
  virtual void validate(const json::json_pointer &ptr, const json &instance,
                        error_handler &e) const = 0;

  static std::shared_ptr<schema> make(json &schema, root_schema *root,
...
class schema_ref : public schema {
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
                error_handler &e) const final {
    if (target_)
...
class root_schema : public schema {
  ...
  struct schema_file {
    ...
    std::map<json::json_pointer, std::shared_ptr<schema_ref>>
        unresolved;  // contains all unresolved references from any other file
                     // seen during parsing
 ...
 public:
  root_schema(schema_loader loader, format_checker format)
  ...
  void insert(const json_uri &uri, const std::shared_ptr<schema> &s) {
    ...
    if (schema != file.schemas.end() &&
        !(file.schemas.key_comp()(uri.pointer(), schema->first))) {
      throw std::invalid_argument("schema with " + uri.to_string() +
                                  " already inserted");
      return;
    }
  ...
  void insert_unknown_keyword(const json_uri &uri, const std::string &key,
                              json &value) {
    auto &file = get_or_create_file(uri.location());
  ...
  std::shared_ptr<schema> get_or_create_ref(const json_uri &uri) {
    ...
    if (schema != file.schemas.end()) return schema->second;
    ...
    if (r != file.unresolved.end() &&
        !(file.unresolved.key_comp()(uri.pointer(), r->first))) {
      return r->second;
    } else {
      return file.unresolved
          .insert(r, {uri.pointer(),
                      std::make_shared<schema_ref>(uri.to_string(), this)})
          ->second;
    }
  }
  ...
  void set_root_schema(json schema) {
    ...
    do {
      ...
      for (auto &file : files_) locations.push_back(file.first);

      for (auto &loc : locations) {
        if (files_[loc].schemas.size() ==
            0) {  // nothing has been loaded for this file
          if (loader_) {
            json sch;
...
class logical_not : public schema {
 ...
 public:
  logical_not(json &sch, root_schema *root,
...
enum logical_combination_types { allOf, anyOf, oneOf };
...
class logical_combination : public schema {
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
    ...
    for (auto &s : subschemata_) {
      ...
      if (is_validate_complete(instance, ptr, e, esub, count)) return;
    }

    // could accumulate esub details for anyOf and oneOf, but not clear how to
    // select which subschema failure to report or how to report multiple such
    // failures
    if (count == 0)
      e.error(ptr, instance,
              "no subschema has succeeded, but one of them is required to "
              "validate");
  }

  // specialized for each of the logical_combination_types
  static const std::string key;
  static bool is_validate_complete(const json &, const json::json_pointer &,
                                   error_handler &, const first_error_handler &,
                                   size_t);

 public:
  logical_combination(json &sch, root_schema *root,
                      const std::vector<nlohmann::json_uri> &uris)
      : schema(root) {
    size_t c = 0;
    for (auto &subschema : sch)
      subschemata_.push_back(
          schema::make(subschema, root, {key, std::to_string(c++)}, uris));
...
template <>
bool logical_combination<allOf>::is_validate_complete(
    const json &, const json::json_pointer &, error_handler &e,
...
bool logical_combination<anyOf>::is_validate_complete(
    const json &, const json::json_pointer &, error_handler &,
    const first_error_handler &, size_t count) {
  return count == 1;
}

template <>
bool logical_combination<oneOf>::is_validate_complete(
    const json &instance, const json::json_pointer &ptr, error_handler &e,
    const first_error_handler &, size_t count) {
  if (count > 1)
    e.error(ptr, instance,
            "more than one subschema has succeeded, but exactly one of them is "
...
class type_schema : public schema {
  ...
  static std::shared_ptr<schema> make(json &schema, json::value_t type,
                                      root_schema *,
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
    ...
    // depending on the type of instance run the type specific validator - if
    // present
    ...
    for (auto l : logic_) l->validate(ptr, instance, e);
    ...
    if (if_) {
      ...
      if (!err) {
        if (then_) then_->validate(ptr, instance, e);
      } else {
        if (else_) else_->validate(ptr, instance, e);
      }
    }
  }

 public:
  type_schema(json &sch, root_schema *root,
              const std::vector<nlohmann::json_uri> &uris)
      : schema(root), type_((uint8_t)json::value_t::discarded + 1) {
    // association between JSON-schema-type and NLohmann-types
    static const std::vector<std::pair<std::string, json::value_t>>
        schema_types = {
      ...
      switch (attr.value().type()) {  // "type": "type"
        ...
        case json::value_t::array:  // "type": ["type1", "type2"]
          for (auto &schema_type : attr.value())
            for (auto &t : schema_types)
              if (t.first == schema_type)
                type_[(uint8_t)t.second] = type_schema::make(
                    sch, t.second, root, uris, known_keywords);
          break;
    ...
    for (auto &key : known_keywords) sch.erase(key);

    // with nlohmann::json float instance (but number in schema-definition) can
    // be seen as unsigned or integer - reuse the number-validator for integer
    // values as well, if they have not been specified explicitly
    if (type_[(uint8_t)json::value_t::number_float] &&
        !type_[(uint8_t)json::value_t::number_integer])
      type_[(uint8_t)json::value_t::number_integer] =
          type_[(uint8_t)json::value_t::number_float];

    // #54: JSON-schema does not differentiate between unsigned and signed
    // integer - nlohmann::json does we stick with JSON-schema: use the
    // integer-validator if instance-value is unsigned
    ...
    attr = sch.find("allOf");
    if (attr != sch.end()) {
      logic_.push_back(std::make_shared<logical_combination<allOf>>(
          attr.value(), root, uris));
      sch.erase(attr);
    }

    attr = sch.find("anyOf");
    if (attr != sch.end()) {
      logic_.push_back(std::make_shared<logical_combination<anyOf>>(
          attr.value(), root, uris));
      sch.erase(attr);
    }
    ...
    attr = sch.find("oneOf");
    if (attr != sch.end()) {
      logic_.push_back(std::make_shared<logical_combination<oneOf>>(
          attr.value(), root, uris));
      sch.erase(attr);
    }
...
#ifndef NO_STD_REGEX
  std::pair<bool, REGEX_NAMESPACE::regex> pattern_{false,
                                                   REGEX_NAMESPACE::regex()};
...
#ifndef NO_STD_REGEX
    if (pattern_.first && !REGEX_NAMESPACE::regex_search(
                              instance.get<std::string>(), pattern_.second))
      e.error(ptr, instance,
              "instance does not match regex pattern: " + patternString_);
#endif

    if (format_.first) {
      if (root_->format_check() == nullptr)
        e.error(ptr, instance,
                std::string("a format checker was not provided but a format "
                            "keyword for this string is present: ") +
                    format_.second);
      else {
        try {
          ...
          e.error(ptr, instance,
                  std::string("format-checking failed: ") + ex.what());
        }
      }
    }
  }
 ...
 public:
  string(json &sch, root_schema *root) : schema(root) {
    ...
    if (attr != sch.end()) {
      ...
      pattern_ = {true,
                  REGEX_NAMESPACE::regex(attr.value().get<std::string>(),
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
      ...
      if (violates_multiple_of(value))
        e.error(ptr, instance,
                "instance is not a multiple of " +
                    std::to_string(multipleOf_.second));
    ...
    if (maximum_.first)
      if ((exclusiveMaximum_ && value >= maximum_.second) ||
        ...
        e.error(
            ptr, instance,
            "instance exceeds maximum of " + std::to_string(maximum_.second));
    ...
    if (minimum_.first)
      if ((exclusiveMinimum_ && value <= minimum_.second) ||
        ...
        e.error(
            ptr, instance,
            "instance is below minimum of " + std::to_string(minimum_.second));
  }
...
class null : public schema {
  void validate(const json::json_pointer &ptr, const json &instance,
                error_handler &e) const override {
    if (!instance.is_null()) e.error(ptr, instance, "expected to be null");
  }
...
class boolean_type : public schema {
  void validate(const json::json_pointer &, const json &,
                error_handler &) const override {}
...
class boolean : public schema {
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
    ...
    if (!true_) {  // false schema
      ...
      //		e.error(ptr, instance, "false-schema required empty
      //array"); 	return;
      //}
...
class object : public schema {
...
#ifndef NO_STD_REGEX
  std::vector<std::pair<REGEX_NAMESPACE::regex, std::shared_ptr<schema>>>
      patternProperties_;
#endif
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
    ...
    for (auto &r : required_)
      if (instance.find(r) == instance.end())
        e.error(ptr, instance,
                "required property '" + r + "' not found in object");

    // for each property in instance
    for (auto &p : instance.items()) {
      if (propertyNames_) propertyNames_->validate(ptr, p.key(), e);
      ...
      if (!a_prop_or_pattern_matched && additionalProperties_) {
        ...
        additionalProperties_->validate(ptr / p.key(), p.value(),
                                        additional_prop_err);
    ...
    for (auto &dep : dependencies_) {
      ...
      if (prop !=
          instance.end())  // if dependency-property is present in instance
        dep.second->validate(ptr / dep.first, instance, e);  // validate
    }
  }

 public:
  object(json &sch, root_schema *root,
         const std::vector<nlohmann::json_uri> &uris)
      : schema(root) {
    ...
    if (attr != sch.end()) {
      for (auto prop : attr.value().items())
        properties_.insert(std::make_pair(
            prop.key(), schema::make(prop.value(), root,
                                     {"properties", prop.key()}, uris)));
      sch.erase(attr);
    }
    ...
    if (attr != sch.end()) {
      for (auto prop : attr.value().items())
        patternProperties_.push_back(std::make_pair(
            REGEX_NAMESPACE::regex(prop.key(),
                                   REGEX_NAMESPACE::regex::ECMAScript),
    ...
    if (attr != sch.end()) {
      additionalProperties_ =
          schema::make(attr.value(), root, {"additionalProperties"}, uris);
      sch.erase(attr);
    }

    attr = sch.find("dependencies");
    if (attr != sch.end()) {
      for (auto &dep : attr.value().items()) switch (dep.value().type()) {
          case json::value_t::array:
          ...
          default:
            dependencies_.emplace(
                dep.key(), schema::make(dep.value(), root,
                                        {"dependencies", dep.key()}, uris));
            break;
        }
      sch.erase(attr);
    }
    ...
    if (attr != sch.end()) {
      propertyNames_ =
          schema::make(attr.value(), root, {"propertyNames"}, uris);
      sch.erase(attr);
    }
  }
};
...
class array : public schema {
 ...
 public:
  array(json &sch, root_schema *root,
        const std::vector<nlohmann::json_uri> &uris)
      : schema(root) {
    ...
    if (attr != sch.end()) {
      if (attr.value().type() == json::value_t::array) {
        ...
        for (auto &subsch : attr.value())
          items_.push_back(
              schema::make(subsch, root, {"items", std::to_string(c++)}, uris));

        auto attr_add = sch.find("additionalItems");
        if (attr_add != sch.end()) {
          additionalItems_ =
              schema::make(attr_add.value(), root, {"additionalItems"}, uris);
          sch.erase(attr_add);
        }
...
std::shared_ptr<schema> type_schema::make(
    json &schema, json::value_t type, root_schema *root,
...
std::shared_ptr<schema> schema::make(json &schema, root_schema *root,
                                     const std::vector<std::string> &keys,
                                     std::vector<nlohmann::json_uri> uris) {
  // append to all URIs the keys for this sub-schema
  for (auto &key : keys)
    for (auto &uri : uris) uri = uri.append(key);
  ...
  if (schema.type() == json::value_t::boolean)
    ...
    auto attr =
        schema.find("$id");  // if $id is present, this schema can be referenced
                             // by this ID as an additional URI
    if (attr != schema.end()) {
      if (std::find(uris.begin(), uris.end(),
                    attr.value().get<std::string>()) == uris.end())
        uris.push_back(uris.back().derive(
            attr.value()));  // so add it to the list if it is not there already
      schema.erase(attr);
    }
    ...
    if (attr != schema.end()) {  // this schema is a reference
      ...
      // so this is the origial URI for this reference, the $ref-value has thus
      // be resolved from it
...
class throwing_error_handler : public error_handler {
  void error(const json::json_pointer &ptr, const json &instance,
             const std::string &message) override {
    throw std::invalid_argument(std::string("At ") + ptr.to_string() + " of " +
                                instance.dump() + " - " + message + "\n");
  }
};
...
json_validator::json_validator(schema_loader loader, format_checker format)
    : root_(std::unique_ptr<root_schema>(new root_schema(loader, format))) {}

// move constructor, destructor and move assignment operator can be defaulted
// here where root_schema is a complete type
